...
- Atributos Woo no CSV usam `pa_cor` e `pa_tamanho` (atributos globais), com colunas `Attribute X visible` e `Attribute X global` preenchidas com `1` no pai e nas variações.
- Valores de atributo são exportados com separador por vírgula (nunca pipe): `Attribute 1 value(s)` usa `color.slug` (ex.: `off-white,preto`) e `Attribute 2 value(s)` usa tamanhos (ex.: `P,M,G`).
- Colunas de estoque adicionadas para import mais previsível: `In stock? = 1`, `Manage stock? = 0`, `Stock` vazio.
- Colunas `SKU` e `Parent` são forçadas como string (com aspas) no CSV para evitar interpretação numérica indevida.
...
### Teste rápido no WooCommerce

1. Exporte o CSV em `/admin` com pelo menos 2 cores e 2 tamanhos.
2. Importe no WooCommerce (produtos variáveis).
3. Verifique no admin do produto importado:
   - Atributos globais `pa_cor` e `pa_tamanho` preenchidos corretamente;
   - Variações sem `Qualquer Cor...` / `Qualquer Tamanho...`.
4. Verifique no front do produto se os seletores de Cor/Tamanho aparecem e alteram a variação normalmente.
